  ...
  const renderBeverages = (bootstrap) => {
    ...
    operationArea.innerHTML = `
      ...
      <section class="card offline-benefit-forms">
        <details><summary><strong>BENEFICIO RRPP</strong><small>Sin cobro · descuenta stock</small></summary><div class="details-content stack-form"><label>Bebida<select data-benefit-beverage>${productOptions}</select></label><div class="form-grid-2"><label>Cantidad<select data-benefit-quantity>${Array.from({ length: 20 }, (_, index) => `<option value="${index + 1}">${index + 1}</option>`).join('')}</select></label><label>Valor de referencia<select data-benefit-price>${priceOptions}</select></label></div><button type="button" class="primary-button full" data-offline-benefit>Guardar beneficio</button></div></details>
    ...
    operationArea.querySelector('[data-offline-benefit]')?.addEventListener('click', () => queueQuickSale({ sale_kind: 'rrpp_benefit', beverage_id: operationArea.querySelector('[data-benefit-beverage]').value, benefit_price: operationArea.querySelector('[data-benefit-price]').value, quantity: operationArea.querySelector('[data-benefit-quantity]').value, payment_method: 'other' }));
